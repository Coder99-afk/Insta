...
class PostServiceTest {
    ...
    void testCreatePost() throws IOException {
        ...
        mockPost.setId(1L); // Set the expected ID here
        mockPost.setCaption(caption);
        when(postRepo.save(any(Post.class))).thenReturn(mockPost);

        when(imageService.saveImage(any(MultipartFile.class), eq(1L))).thenReturn("saved/image/path");
        ...
        assertEquals(1, result.getImage().size());
        assertEquals("test.jpg", result.getImage().get(0).getImageName());
        assertEquals("image/jpeg", result.getImage().get(0).getImageType());
        verify(postRepo, times(2)).save(any(Post.class));
        verify(imageRepo, times(1)).save(any(Image.class));
        verify(imageService, times(1)).saveImage(mockFile, 1L);
    }
